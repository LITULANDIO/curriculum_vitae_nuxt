<template>
  <div ref="terminalPane" class="inset-0 flex items-center justify-center">
    <div
      ...
      class="mx-auto w-full max-w-[600px] px-[10px] h-auto absolute z-10 terminal-container"
      @click="focusHiddenInput"
      @touch="focusHiddenInput"
    >
      <div
        :class="[
          'relative w-full rounded-[10px] flex flex-col overflow-hidden font-mono text-left bg-transparent h-[350px] md:h-[420px]',
          textBodyClass,
          shadowClass,
        ]"
      >
        <div
          :class="[
            'w-full h-[30px] rounded-t-[10px] flex items-center justify-between px-[10px] box-border z-10 terminal-body',
            bgHeaderClass,
          ]"
        >
          ...
          <div :class="['flex-1 text-center font-bold', textHeaderClass]">
            {{ $t("terminal.header") }}
          </div>
        </div>
        <div
          :class="[
            'absolute top-[25px] left-0 right-0 bottom-0 rounded-b-[10px] z-0',
            bgBodyClass,
          ]"
        ></div>
        <div
          ref="outputRef"
          class="relative z-20 flex-1 overflow-y-auto p-2.5 whitespace-pre-wrap pb-2.5 text-xs md:text-base max-h-[350px]"
        >
        ...
        <div
          ref="inputField"
          class="relative z-20 flex items-center p-2.5 text-xs md:text-base field"
        >
          ...
          />
          <span
            class="flex-1 outline-none overflow-hidden text-[#00ff00] whitespace-nowrap"
          >
            {{ currentInput }}
          </span>
          <span
            :class="[
              'inline-block w-[10px] bg-[#00ff00] text-[#00ff00] ml-1.5',
              cursorClass,
            ]"
            >|</span
          >
...
const bgBodyClass = computed(() =>
  isDarkTheme.value ? "bg-black" : "bg-white"
);
const bgHeaderClass = computed(() =>
  isDarkTheme.value ? "bg-white" : "bg-black"
);
const textBodyClass = computed(() =>
  isDarkTheme.value ? "text-white" : "text-black"
);
const textHeaderClass = computed(() =>
  isDarkTheme.value ? "text-black" : "text-white"
);
const shadowClass = computed(() =>
  isDarkTheme.value ? "shadow-shadow-white" : "shadow-shadow-dark"
);
const cursorClass = computed(() =>
  cursorActive.value ? "opacity-100" : "opacity-0"
);

const typeCommand = () => {
  if (commandIndex.value < commands.value.length) {
    if (charIndex.value < commands.value[commandIndex.value].length) {
      currentInput.value += commands.value[commandIndex.value].charAt(
        charIndex.value
      );
      charIndex.value++;
...
const translatedOutputLines = computed(() => {
  return outputLines.value.map((line) => {
    if (
      line == "¡Hola!👋 me llamo Carles" ||
      line == "Hello!👋 My name is Carles"
    ) {
      return t("terminal.commands1");
    } else if (
      line == "Bienvenido a mi currículum interactivo" ||
      line == "Welcome to my interactive resume"
    ) {
      return t("terminal.commands2");
    } else if (
      line == "Soy un apasionado del diseño y desarrollo web 👨🏻‍💻" ||
      line == "I am passionate about web design and development 👨🏻‍💻"
    ) {
      return t("terminal.commands3");
    } else if (
      line == "Mi especialidad es el Front end" ||
      line == "My specialty is Frontend"
    ) {
      return t("terminal.commands4");
    } else if (
      line == "Comando no reconocido" ||
      line == "Command not recognised"
    ) {
